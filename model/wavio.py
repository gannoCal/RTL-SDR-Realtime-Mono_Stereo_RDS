...
if __name__ == "__main__":
    ...
    rf_coeff = signal.firwin(rf_taps, \
                            rf_Fc/(rf_Fs/2), \
                            window=('hann'))
    indx1 = np.arange(0,len(rf_coeff),1,int)
    indxd_data1 = np.column_stack((indx1,rf_coeff))
    np.savetxt('test.out', indxd_data1, delimiter=',', fmt='%d  %1.5e')
    ...
    audio_coeff = signal.firwin(numTaps,Fc/(Fs/2), window='hann') # to be updated by you during in-lab
    indx = np.arange(0,len(audio_coeff),1,int)
    indxd_data = np.column_stack((indx,audio_coeff))
    
    np.savetxt('test2.out', indxd_data, delimiter=',', fmt='%d  %1.5e')
    ...
    wavfile.write("../data/test_proj25.wav", \
                int(48000), \
